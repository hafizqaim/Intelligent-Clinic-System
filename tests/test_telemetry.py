...
async def patient_b(client_doctor_b: httpx.AsyncClient) -> str:
    ...
    r = await client_doctor_b.post("/api/patients/", json={
        "name": "Tele Patient B",
...
class TestIngest:
    async def test_ingest_normal(self, client_doctor: httpx.AsyncClient, patient_a: str):
        r = await client_doctor.post("/api/telemetry/ingest", json={
    ...
    async def test_ingest_anomalous(self, client_doctor: httpx.AsyncClient, patient_a: str):
    ...
    async def test_ingest_unauthenticated(self, client_anon: httpx.AsyncClient):
        r = await client_anon.post("/api/telemetry/ingest", json={
...
class TestReadings:
    async def test_readings_returns_ingested(
        ...
        await client_doctor.post("/api/telemetry/ingest", json={
            "patient_id": patient_a,
            "clinic_id": CLINIC_A_ID,
            "heart_rate": 65,
